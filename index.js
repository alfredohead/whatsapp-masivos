...
// Espera readiness (autenticado o listo) con timeout
...
client.on('auth_failure', msg => { isReady = false; console.error('❌ Error autenticación:', msg); });
...
app.post('/enviar', async (req, res) => {
  ...
  try {
    ...
    if (m.includes('invalid wid'))              return res.status(400).json({ error: 'ID WhatsApp inválido', detalles: m });
...
app.use((err, req, res, next) => { console.error('Error servidor:', err); res.status(500).json({ error: 'Error del servidor' }); });
